...
"""
We have tried to use wali ans wali_gp, but seen apparent performance decline."""
...
def wali_gp(disc_fake, disc_real, gradient_penalty, gen_params, disc_params, lr=1e-4, beta1=0.5, beta2=0.999,
            rec_loss=None, cls_real_loss=None, cls_fake_loss=None, corr_loss=None, adam=False):
    gen_loss = -tf.reduce_mean(disc_fake) + tf.reduce_mean(disc_real)
    disc_loss = tf.reduce_mean(disc_fake) - tf.reduce_mean(disc_real)
...
disc_real, clogits_real, _ = discriminator(real_x, q_z)
disc_fake, clogits_fake, de_z = discriminator(fake_x, p_z)
...
'''correlation loss'''
corr_loss = None
if opt.MODE_BGAC == 'ali':
    gen_loss, disc_loss, gen_train_op, disc_train_op = ali(disc_fake, disc_real,
                                                            ...
                                                            corr_loss=corr_loss,
                                                            adam=True)
elif opt.MODE_BGAC == 'wali':
    gen_loss, disc_loss, clip_disc_weights, gen_train_op, disc_train_op, clip_ops = wali(disc_fake, disc_real,
                                                           gen_params + ext_params, disc_params,
                                                           lr=5e-5,
                                                           rec_loss=rec_loss,
                                                           cls_real_loss=cls_real_loss, cls_fake_loss=cls_fake_loss,
                                                           corr_loss=corr_loss,
                                                           adam=False)
elif opt.MODE_BGAC == 'wali_gp':
    alpha = tf.random.uniform(
        shape=[opt.BATCH_SIZE, 1],
        minval=0.,
        maxval=1.
    )
    ...
    gen_loss, disc_loss, gen_train_op, disc_train_op = wali_gp(disc_fake, disc_real, gradient_penalty,
                                                            gen_params + ext_params, disc_params,
                                                            lr=opt.LR, beta1=opt.BETA1, beta2=opt.BETA2,
                                                            rec_loss=rec_loss,
                                                            cls_real_loss=cls_real_loss, cls_fake_loss=cls_fake_loss,
